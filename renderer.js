...
function setActiveTab(id) {
  ...
  if (wv && zoomLevels[id]) wv.setZoomFactor(zoomLevels[id]);
}
...
async function checkForUpdates() {
  const url = settings.updateUrl || $('#setting-update-url')?.value?.trim();
  const statusEl = $('#update-status');
  if (!url) {
    toast('⚠ Set an Update URL in Settings first.');
    if (statusEl) { statusEl.textContent = '⚠ No Update URL configured.'; statusEl.className = 'update-status error'; statusEl.classList.remove('hidden'); }
    return;
  }
  ...
  try {
    ...
    if (!data.version) throw new Error('Invalid version.json format');

    const isNewer = compareVersions(data.version, currentVersion) > 0;
    if (isNewer) {
      if (statusEl) {
        statusEl.innerHTML = `🚀 Update available: <b>v${data.version}</b>${data.notes ? `<br>${escHtml(data.notes)}` : ''}<br><a class="update-dl-link" href="#">Download Update</a>`;
        statusEl.className = 'update-status success';
        statusEl.classList.remove('hidden');
        statusEl.querySelector('.update-dl-link')?.addEventListener('click', (e) => {
          e.preventDefault();
          window.nexus.openExternal(data.url);
        });
      }
    } else {
      if (statusEl) { statusEl.textContent = '✓ You are on the latest version (v' + currentVersion + ')'; statusEl.className = 'update-status ok'; statusEl.classList.remove('hidden'); }
      toast('✓ Already on latest version!');
    }
  } catch (err) {
    if (statusEl) { statusEl.textContent = '✕ Could not check for updates: ' + err.message; statusEl.className = 'update-status error'; statusEl.classList.remove('hidden'); }
  }
}
...
document.addEventListener('keydown', (e) => {
  ...
  // ── F keys ───────────────────────────────────────────────────────────────
  if (key === 'F5')  { e.preventDefault(); getActiveWV()?.reload(); return; }
  if (key === 'F11') {
    e.preventDefault();
  ...
  // ── Escape — close find bar or modal ─────────────────────────────────────
  if (key === 'Escape') {
